...
/// Generate a terminal buffer name of the form "term://$CWD//$PID:$CMD" in
/// buf.
void buf_name_for_term(char_u* buf, size_t size, const char_u* cwd, varnumber_T pid, const char* cmd) {
  snprintf((char*)buf, size, "term://%s//%li:%s", cwd, pid, cmd);
}
